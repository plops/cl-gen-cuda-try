...
void cuda_list_attributes(int cuda_dev) {
  {
    ...
    printf(
        "cudaDevAttrTotalConstantMemory.............. = %12d (Memory available "
        "on device for __constant__variables in a CUDA C kernel in bytes)\n",
        val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrWarpSize, cuda_dev));
    printf("cudaDevAttrWarpSize......................... = %12d (Warp size in "
           "threads)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrMaxPitch, cuda_dev));
    printf("cudaDevAttrMaxPitch......................... = %12d (Maximum pitch "
           "in bytes allowed by the memory copyfunctions that involve memory "
           "regions allocated through cudaMallocPitch())\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrMaxTexture1DWidth, cuda_dev));
    printf("cudaDevAttrMaxTexture1DWidth................ = %12d (Maximum 1D "
           "texture width)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrMaxTexture1DLinearWidth, cuda_dev));
    printf("cudaDevAttrMaxTexture1DLinearWidth.......... = %12d (Maximum width "
           "for a 1D texture boundto linear memory)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf("cudaDevAttrMaxTexture2DWidth................ = %12d (Maximum 2D "
           "texture width)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrMaxTexture2DHeight, cuda_dev));
    printf("cudaDevAttrMaxTexture2DHeight............... = %12d (Maximum 2D "
           "texture height)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrMaxTexture2DLinearWidth, cuda_dev));
    printf("cudaDevAttrMaxTexture2DLinearWidth.......... = %12d (Maximum width "
           "for a 2D texture boundto linear memory)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf("cudaDevAttrMaxTexture2DLinearPitch.......... = %12d (Maximum pitch "
           "in bytes for a 2D texturebound to linear memory)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf("cudaDevAttrMaxTexture1DLayeredWidth......... = %12d (Maximum 1D "
    ...
    printf("cudaDevAttrMaxTexture2DLayeredWidth......... = %12d (Maximum 2D "
           "layered texture width)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrMaxTexture2DLayeredHeight, cuda_dev));
    printf("cudaDevAttrMaxTexture2DLayeredHeight........ = %12d (Maximum 2D "
    ...
    printf("cudaDevAttrMaxSurface2DLayeredWidth......... = %12d (Maximum 2D "
           "layered surface width)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrMaxSurface2DLayeredHeight, cuda_dev));
    printf("cudaDevAttrMaxSurface2DLayeredHeight........ = %12d (Maximum 2D "
    ...
    printf("cudaDevAttrClockRate........................ = %12d (Peak clock "
           "frequency in kilohertz)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrTextureAlignment, cuda_dev));
    printf("cudaDevAttrTextureAlignment................. = %12d (Alignment "
    ...
    printf(
        "cudaDevAttrTexturePitchAlignment............ = %12d (Pitch alignment "
        "requirement for 2D texturereferences bound to pitched memory)\n",
        val);
    check_cuda_errors(
    ...
    printf("cudaDevAttrMultiProcessorCount.............. = %12d (Number of "
           "multiprocessors on the device)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrKernelExecTimeout, cuda_dev));
    printf("cudaDevAttrKernelExecTimeout................ = %12d (1 if there is "
    ...
    printf(
        "cudaDevAttrCanMapHostMemory................. = %12d (1 if the device "
        "can map host memory into theCUDA address space, or 0 if not)\n",
        val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrComputeMode, cuda_dev));
    printf("cudaDevAttrComputeMode...................... = %12d (Compute mode "
           "is the compute mode that the device iscurrently in. Available "
    ...
    printf("cudaDevAttrEccEnabled....................... = %12d (1 if error "
    ...
    printf("cudaDevAttrPciDeviceId...................... = %12d (PCI device "
    ...
    printf("cudaDevAttrMemoryClockRate.................. = %12d (Peak memory "
           "clock frequency in kilohertz)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrGlobalMemoryBusWidth, cuda_dev));
    printf("cudaDevAttrGlobalMemoryBusWidth............. = %12d (Global memory "
           "bus width in bits)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrL2CacheSize, cuda_dev));
    printf("cudaDevAttrL2CacheSize...................... = %12d (Size of L2 "
    ...
    printf("cudaDevAttrComputeCapabilityMajor........... = %12d (Major compute "
           "capability version number)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrComputeCapabilityMinor, cuda_dev));
    printf("cudaDevAttrComputeCapabilityMinor........... = %12d (Minor compute "
           "capability version number)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf("cudaDevAttrGlobalL1CacheSupported........... = %12d (1 if device "
           "supports caching globals in L1cache, 0 if not)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
        &val, cudaDevAttrLocalL1CacheSupported, cuda_dev));
    printf("cudaDevAttrLocalL1CacheSupported............ = %12d (1 if device "
           "supports caching locals in L1cache, 0 if not)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf("cudaDevAttrManagedMemory.................... = %12d (1 if device "
           "supports allocating managed memory, 0 if not)\n",
           val);
    check_cuda_errors(
        cudaDeviceGetAttribute(&val, cudaDevAttrIsMultiGpuBoard, cuda_dev));
    printf("cudaDevAttrIsMultiGpuBoard.................. = %12d (1 if device "
           "is on a multi-GPU board, 0 if not)\n",
           val);
    check_cuda_errors(cudaDeviceGetAttribute(
    ...
    printf(
        "cudaDevAttrHostNativeAtomicSupported........ = %12d (1 if the link "
    ...
    printf("cudaDevAttrMaxSharedMemoryPerBlockOptin..... = %12d (Maximum per "
  ...
  check_cuda_errors(cudaDeviceSetCacheConfig(cudaFuncCachePreferShared));
  {
    ...
    cudaMemcpyAsync(d_c, c, (N * sizeof(int)), cudaMemcpyHostToDevice, 0);
    {
      ...
      check_cuda_errors(cudaEventSynchronize(stop));
      {
        ...
        printf("executing some code took %f ms.\n", time);
